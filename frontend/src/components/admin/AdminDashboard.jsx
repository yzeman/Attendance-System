...
const AdminDashboard = () => {
  ...
  // Fetch semester toggle preference from database
  const fetchSemesterTogglePreference = async () => {
    try {
      const { data, error } = await supabase
        .from('users')
        .select('semester_toggle_preference')
        .eq('id', user.id)
        .single();
      ...
      if (data && data.semester_toggle_preference !== null) {
        setSecondSemesterEnabled(data.semester_toggle_preference);
      }
    } catch (error) {
      console.error('Error fetching semester preference:', error);
    }
  ...
  // Save semester toggle preference to database
  const saveSemesterTogglePreference = async (value) => {
    try {
      const { error } = await supabase
        .from('users')
        .update({ semester_toggle_preference: value })
        .eq('id', user.id);
  ...
  return (
    <Container fluid className="mt-4">
      <div className="animate-fade-in">
        ...
        {/* Semester Toggle - SAVED PERMANENTLY */}
              ...
              <div className="d-flex justify-content-end align-items-center">
                <Form.Check
                  ...
                  onChange={async (e) => {
                    ...
                    setSecondSemesterEnabled(newValue);
                    await saveSemesterTogglePreference(newValue);
                    // Refresh courses to reflect changes
                    await fetchAllData();
                  }}
                />
              </div>
